...
for df in [train_df, test_df]:
    df['TransactionAmtPerDay'] = df['TransactionAmt']/df['D3']
...
# ## . Statistical features on categories related to uid
# 对与uid有关的各个类别做统计特征，包括均值、中位数、标准差、数量、zscore
...
i_cols = ['uid1','uid2','uid3','card2','card3','addr2']
for period in periods:
    for col in i_cols:
...
with open('./data/train_1.1.pkl', 'wb') as f:
    pickle.dump(train_df, f)
with open('./data/test_1.1.pkl', 'wb') as f:
    pickle.dump(test_df, f)
...
test_predictions = make_predictions(train_df, test_df, feature_cols, labels, lgb_param, NFOLDS=6)


# In[65]:


test_predictions.to_csv(f"./sub/sub_{datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')}.csv", index=False)
