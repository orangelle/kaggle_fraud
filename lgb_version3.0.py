...
def make_predictions_kf(train_df, test_df, feature_cols, target, param, NFOLDS=2):
    ...
    test_pred_prob = np.zeros(test_df.shape[0])
    oof_pred_prob = np.zeros(train_df.shape[0])
...
def negative_sampling_bagging(train_df, test_df, feature_cols, lgb_param, times=5, NFOLDS=10):
    
    test_predictions = test_df[['TransactionID']]
    test_predictions['isFraud'] = 0
    ...
    for i in range(times):
        ...
        train_neg = train_neg.sample(int(train_df.shape[0] * 0.2), random_state=SEED*times)
        ...
        test_predictions['isFraud'] += make_predictions_kf(train_df, test_df, feature_cols, labels, lgb_param, NFOLDS)['isFraud']/times
...
if LOCAL_TEST:
    ...
    lgb_param['n_estimators'] = 1750
    lgb_param['early_stopping_rounds'] = 100
    # test_predictions = make_predictions_gkf(train_df, test_df, feature_cols, labels, lgb_param, NFOLDS=6)
    if NEG_SAM_BAGGING:
        test_predictions = negative_sampling_bagging(train_df, test_df, feature_cols, lgb_param, times = 6, NFOLDS=2)
    else:
