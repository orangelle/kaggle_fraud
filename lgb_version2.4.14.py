...
def add_grouped_time_delta_features(train_df, test_df, time_col, group_cols, shifts):    
    '''
    ...
    for df in [train_df, test_df]:
        ...
        for shift in shifts:
            ...
            df[feat_name] = df.groupby(group_cols)[time_col].shift(shift) - df[time_col]
...
######################### Create other possible combination of categorical feature
for df in [train_df, test_df]:
    df['addr1_DT_D'] = df['addr1'].astype(str)+'_'+df['DT_D'].astype(str)
...
cate_cols = ['card1', 'card2', 'card3', 'card5', 'uid1', 'uid2', 'uid3', 'uid4','uid5','addr1_DT_D']
...
rm_cols = [
    ...
    'card1', 'card2',
    'uid1', 'uid2', 'uid3', 'uid4','uid5','addr1_DT_D',
    'addr1',
...
cate_cols = ['card2', 'card3', 'card5', 'card4', 'card6', 'addr1', 'addr2', 'ProductCD', 'P_emaildomain', 'R_emaildomain', 'R_emaildomain_prefix', 
...
if LOCAL_TEST:
    ...
    test_predictions = make_predictions_kf(train_df, test_df, feature_cols,cate_cols, labels, lgb_param)
else:
    ...
    test_predictions = make_predictions_kf(train_df, test_df, feature_cols, cate_cols, labels, lgb_param, NFOLDS=10)
