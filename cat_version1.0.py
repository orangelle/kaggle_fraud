...
def make_predictions_kf(train_df, test_df, feature_cols, cate_cols, labels, param, NFOLDS=2):
    ...
    for i, (train_idx, valid_idx) in enumerate (kf.split(train_values, labels)):
        ...
        if LOCAL_TEST:
            ...
            # Training
            clf.fit(train_x, train_y, eval_set=[(valid_x, valid_y), (test_values, test_labels)], cat_features = cate_cols, use_best_model=True)
            #clf.fit(train_x, train_y, eval_set=(test_values, test_labels), cat_features = cate_cols)
            
            # Prediction
            valid_pred_prob = clf.predict_proba(valid_x)[:,1]
            oof_pred_prob[valid_idx] =  valid_pred_prob
            # print('val logloss: ', log_loss(valid_y, valid_pred_prob))
            print('val auc: ', roc_auc_score(valid_y, valid_pred_prob))
            
            cur_test_pred_prob = clf.predict_proba(test_values)[:,1]
            ...
            feature_imp['fold_{}'.format(i)] = clf.feature_importances_
            
        else:   
            clf = CatBoostClassifier(**param)

            # Training
            clf.fit(train_x, train_y, eval_set=(valid_x, valid_y), cat_features = cate_cols, use_best_model=True)
            ...
            test_pred_prob += clf.predict_proba(test_values)[:,1]/kf.n_splits
...
if LOCAL_TEST:
    cat_param['learning_rate'] = 0.07
    cat_param['n_estimators'] = 5000
    test_predictions, oof_pred_prob = make_predictions_kf(train_df, test_df, feature_cols, cate_cols, labels, cat_param)
else:
    lgb_param['learning_rate'] = 0.04
    lgb_param['n_estimators'] = 4000
    ...
    test_predictions, oof_pred_prob = make_predictions_kf(train_df, test_df, feature_cols, cate_cols, labels, cat_param, NFOLDS=10)
    test_predictions.to_csv(f"./stacking/sub_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_.csv", index=False)
                            
    oof_df = train_df[['TransactionID']]
    oof_df['isFraud'] = oof_pred_prob
    oof_df.to_csv(f"./stacking/oof_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_.csv", index=False)
